...
├── cherry.html     # 버전 1. 벚꽃 (분홍)
├── hydrangea.html  # 버전 2. 수국 (하늘색)
├── lavender.html   # 버전 3. 라벤더 (보라)
├── summer.html     # 버전 4. 여름 · 바다 (하늘색 · 민트)
...
내용(글·사진·연락처·계좌·달력·지도·BGM)과 기능은 **모든 파일이 완전히 동일**하고, 색과 장식만 다릅니다.
꽃 버전 3종(벚꽃·수국·라벤더)은 구조가 같고 **꽃 그림과 색만** 다릅니다.

| 파일 | 메인 색 | 꽃 · 장식 |
|---|---|---|
| `cherry.html` | 분홍 | 벚꽃 — 끝이 갈라진 5장 꽃잎, 코너 벚꽃 가지, 흩날리는 꽃잎 |
| `hydrangea.html` | 하늘색 | 수국 — 낱꽃이 공처럼 모인 송이, 파랑·연보라 섞임 |
| `lavender.html` | 보라 | 라벤더 — 이삭 모양 줄기 다발, 초록 잎 |
| `summer.html` | 하늘색 · 민트 | 꽃 대신 파도 — 섹션 경계마다 물결, 바다빛 커버 |

세 꽃 버전 공통: 커버 사진 하단이 배경색으로 페이드, 세리프 이탤릭 영문 라벨, 단색 SVG 전화·문자 아이콘, 화면에 은은히 떨어지는 꽃잎(모션 최소화 설정 시 자동으로 꺼짐).

**고르는 법** — 브라우저에서 네 파일을 열어 비교한 뒤, 마음에 드는 파일을 `index.html` 로 바꾸면 됩니다.

```bash
cp index.html index-original.html   # 기존 버전 백업
cp cherry.html index.html           # 예: 벚꽃 버전 채택
```

> 배포 주소(`.../wedding-invitation/`)는 항상 `index.html` 을 보여줍니다. 나머지 파일도 같이 올려두면 `.../wedding-invitation/lavender.html` 처럼 개별 주소로도 열 수 있어, 가족끼리 비교할 때 편합니다.

**주의** — 채택한 뒤 내용을 고칠 때는 **그 파일 하나만** 고치면 됩니다. 파일들은 서로 연결돼 있지 않아서, 예를 들어 계좌번호를 `index.html` 에만 넣으면 나머지 파일에는 반영되지 않습니다. 헷갈리지 않게 **버전을 정한 뒤 안 쓰는 파일은 지우는 걸 권합니다.**
